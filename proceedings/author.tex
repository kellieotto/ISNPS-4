...
\abstract{
   ...
   inaccurate, by an amount that may matter for statistical inference.
    ...
    The algorithms many packages use to generate random samples involve the false assumption
    that PRNGs produce IID uniform outputs.
...
%    but some of those generate the random integers by multiplying a pseudo-random number by a constant and taking the floor: another mathematically correct method that fails in practice.
    ...
    such as generating a random sample by randomly permuting the population and taking the first $k$ items 
...
Other sampling algorithms require uniformly distributed integers (as opposed to the approximately $U[0,1)$ PRNG outputs) as input, but many software packages generate
pseudo-random integers using a truncation method that does not yield uniformly distributed integers, even
...
\begin{tabular}[h]{p{4.5cm}p{2.4cm}p{4cm}p{2cm}}
\hline\noalign{\smallskip}
...
We have been searching for empirical problems large enough to matter in 
...
R should replace the truncation algorithm it uses to generate random integers in the \texttt{sample} function (and other functions) with the more precise bit masking algorithm, as discussed
