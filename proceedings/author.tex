...
\abstract{
\begin{enumerate}
    \item Pigeonhole arguments shows that PRNGs cannot be adequate
    \item Some sampling algorithms are better than others: ones that use only $k$ PRNs to sample are better than those that use $n$ when $k \ll n$
    \item Common software has these issues
    \item ``textbook'' method for drawing SRS is not good
    \item ``textbook'' method for generating random integers is not good
\end{enumerate}
}
